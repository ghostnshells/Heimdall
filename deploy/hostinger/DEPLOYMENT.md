# Panoptes - Hostinger VPS Deployment Guide
...
```bash
cd /var/www
git clone <your-repo-url> panoptes
cd panoptes
npm install
```
...
Create `/etc/nginx/sites-available/panoptes`:
...
```bash
ln -s /etc/nginx/sites-available/panoptes /etc/nginx/sites-enabled/
nginx -t
...
pm2 logs panoptes       # View live logs
pm2 restart panoptes    # Restart the app
pm2 stop panoptes       # Stop the app
...
npm install
npm run build
pm2 restart panoptes
```

## Troubleshooting

- **502 Bad Gateway**: Check `pm2 logs panoptes` — the Node process may have crashed
